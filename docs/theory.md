...
- 计算对比损失
    ...
    - \(\tau\)：温度参数，控制对困难负样本的惩罚强度。
        - 温度参数越接近于1，相似度的权重越小，无法学习到差异；越接近于0，相似度的权重越大，学习的更精细，更尖锐，不能提取到关键的特征。
    ...
    $$
    - 损失函数解释：分子表示正样本之间的相似度，分子越大，意味着正样本之间在特征空间中靠得越近，也就是模型的“对比”效果更好。训练的过程就是正样本经过编码器处理后在特征空间靠得越来越近的过程。
    - 一句话总结：损失函数表示了正样本在样本空间中靠近的程度
### SimCLR实现注意事项
- 固定随机种子：保证实验的可复现性
## MoCo 模型
